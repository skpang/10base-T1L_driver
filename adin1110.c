...
/* MDIO_OP codes */
#define ADIN1110_MDIO_OP_AD			0x0
...
#define ADIN1110_MDIO_DEV_ADDR_DIGIO_PINMUX	0x1E
#define ADIN1110_MDIO_REG_ADDR_DIGIO_PINMUX	0x8C56
#define ADIN1110_MDIO_REG_VALUE_DIGIO_PINMUX	0x00F0

#define ADIN1110_MDIO_DEV_ADDR_LED_CNTRL	0x1E
#define ADIN1110_MDIO_REG_ADDR_LED_CNTRL	0x8C82
#define ADIN1110_MDIO_REG_VALUE_LED_CNTRL	0x9080
...
static int adin1110_mdio_write_c45_reg(struct mii_bus *bus, int phy_id,
			       int reg, u16 c45_addr, u16 reg_val)
{
	struct adin1110_priv *priv = bus->priv;
	int ret;

	u32 val = 0;
	val |= FIELD_PREP(ADIN1110_MDIO_OP, ADIN1110_MDIO_OP_AD);
	val |= FIELD_PREP(ADIN1110_MDIO_ST, 0x0);
	val |= FIELD_PREP(ADIN1110_MDIO_PRTAD, phy_id);
	val |= FIELD_PREP(ADIN1110_MDIO_DEVAD, reg);
	val |= FIELD_PREP(ADIN1110_MDIO_DATA, c45_addr);

	/* write the clause 45 address value */
	mutex_lock(&priv->lock);
	ret = adin1110_write_reg(priv, ADIN1110_MDIOACC, val);
	mutex_unlock(&priv->lock);
	if (ret < 0)
	...
	/* write the clause 45 data value */
	mutex_lock(&priv->lock);
	ret = adin1110_write_reg(priv, ADIN1110_MDIOACC, val);
	mutex_unlock(&priv->lock);
	if (ret < 0)
...
static int adin1110_probe(struct spi_device *spi)
{
	...
	/* LED_1 PinMux */
	adin1110_mdio_write_c45_reg(priv->mii_bus, ADIN1110_PHY_ID, ADIN1110_MDIO_DEV_ADDR_DIGIO_PINMUX,
		ADIN1110_MDIO_REG_ADDR_DIGIO_PINMUX, ADIN1110_MDIO_REG_VALUE_DIGIO_PINMUX);
	/* LED 1 ON when transmit level is 2.4Vpp */
	adin1110_mdio_write_c45_reg(priv->mii_bus, ADIN1110_PHY_ID, ADIN1110_MDIO_DEV_ADDR_LED_CNTRL,
		ADIN1110_MDIO_REG_ADDR_LED_CNTRL, ADIN1110_MDIO_REG_VALUE_LED_CNTRL);
